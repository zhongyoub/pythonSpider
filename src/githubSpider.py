...
import json
'''
def get_statics(owner, name , headers):
    url="http://api.github.com/repos/{owner}/{name}/stats/contributors?page=2&per_page=100".format(owner=owner, name=name)
...
def get_results(search,headers):
    url='http://api.github.com/search/repositories?q={search}&page=4&per_page=100&sort=starts&order=desc'.format(search=search)
    req=Request(url,headers=headers)
    response=urlopen(req).read()
    result=json.loads(response.decode())
    return result
